...
First, we created a base case where we ran a simple t-test on difference in means between treatment and control, and the treatment effect is constant for everyone. Then we used the mean and variance from the pilot pre-survey for self-reported TV since the Netflix data was minimal. Since the pilot was so short, we did not get a good estimate of the treatment effect. Instead, run across a variety of treatment effects and check the power value.
...
In terms of randomization once we had our set list of participants from the pre-survey, we used a simple random assignment to get half of our subjects into control and the other half in treatment in an effort to prevent against any selection bias. For a check, we did a covariate balance check and found that the randomization had worked - there were no statistically significant differences between the control and treatment group nor did the F-test of joint significance return anything of significance. 
...
The below shows us that there were only 5 participants who used the netflix extension and do not share a profile. This is simply not enough data, so it was unusable.

```{r}
all_data$extension <- is.na(as.numeric(all_data$netflixMin))
NFmod <- lm(reportedhrs_P/2 ~ Assigned + source + NFLXreading + extension, data=all_data[all_data$shareProfile=="no",])
summary(NFmod)

hist(na.omit(all_data$NFLXreading[all_data$shareProfile=="no"]))
```

In the below plots we see that participating in the study did not affect the control group or the non-compliers in treatment.
Though we recognize that asking the participants in the final survey if they ever received the treatment email acts as a poor proxy for identifying compliers, it allows us to calculate some somewhat-useful values for alpha and CACE (see below). We control for the only statistically significant covariate, previous hours/week.

```{r}
boxplot(all_data$reportedhrs_P[all_data$Assigned==0]/2,
        all_data$hoursTV[all_data$Assigned==0],
...
boxplot(all_data$reportedhrs_P[all_data$Assigned==1 & all_data$Treated==1]/2,
        all_data$hoursTV[all_data$Assigned==1 & all_data$Treated==1],
...
# The below section considers before/after analysis for compliers and non-compliers,
# which produces an inaccurate view.
# BNC <- na.omit(all_data$hoursTV[all_data$Assigned==1 & all_data$Treated!=1])
# BC <- na.omit(all_data$hoursTV[all_data$Assigned==1 & all_data$Treated==1])
# ANC <- na.omit(all_data$reportedhrs_P[all_data$Assigned==1 & all_data$Treated!=1]/2)
# AC <- na.omit(all_data$reportedhrs_P[all_data$Assigned==1 & all_data$Treated==1]/2)
# print("ATE for Non-Compliers:")
# (ATENC <- mean(ANC)-mean(BNC))
# print("ATE for Compliers:")
# (ATEC <- mean(AC)-mean(BC))

print("Overall ATE:")
(ATE <- lm(reportedhrs_P/2 ~ Assigned + hoursTV, all_data)$coefficients[2])
print("Alpha (rate of compliance):")
(alpha <- length(all_data$hoursTV[all_data$Assigned==1 & all_data$Treated==1]) / length(all_data$hoursTV[all_data$Assigned==1]))
print("CACE:")
(CACE <- ATE/alpha)
```
...
**Attrition**
We witnessed high levels of attrition through out the experiment, starting with 136 potential participants and ending up with 65 actually completing both pre and post surveys. From the time the participants were assigned to treatment and control groups, we saw 17% attrition in the treatment group and 23% in the control. When we regressed completion on treatment group, we found there was so statistical evidence to suggest that attrition was related to treatment group (p-value of .475). Further, ex-ante covariate balance check fails to reject null of no joint significance of covariates predicting treatment assignment.  
